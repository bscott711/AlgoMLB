...
TEAM_NAME_MAP = {
    "Arizona Diamondbacks": "AZ", "ARI": "AZ", "AZ": "AZ", "ARZ": "AZ", "Arizona": "AZ",
    ...
    "Cleveland Guardians": "CLE", "CLE": "CLE", "Cleveland Indians": "CLE",
    ...
    "Oakland Athletics": "OAK", "OAK": "OAK", "Athletics": "OAK", "Oakland": "OAK",
    ...
    "Washington Nationals": "WSH", "WSH": "WSH", "Wats": "WSH"
}
...
def repair_ballparks():
    ...
    with session_factory() as session:
        ...
        stmt = select(GameResultORM).where(GameResultORM.ballpark_id == None)
        ...
        for game in games:
            ...
            if not target_abbrev:
                ...
                target_abbrev = next((v for k, v in TEAM_NAME_MAP.items() if k.lower() in game.home_team.lower()), None)
            
            if target_abbrev and target_abbrev in abbrev_to_id:
                game.ballpark_id = abbrev_to_id[target_abbrev]
                repaired += 1
            else:
                logger.warning(f"❌ Could not match ballpark for home team: {game.home_team} (Game {game.game_id})")
        
        if repaired > 0:
            session.commit()
            logger.success(f"✅ Successfully repaired ballpark_id for {repaired} games.")
        else:
