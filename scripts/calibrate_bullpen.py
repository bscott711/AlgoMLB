...
class BullpenCalibrationObjective:
    def __init__(self, game_pks, pa_model_version="v1.2"):
        ...
        logger.info(
            f"Pre-loading and precomputing {len(game_pks)} games for calibration..."
        )
        self.contexts = []
        self.actuals = []
...
def run_calibration():
    ...
    query = (
        ...
        .limit(100)
    )  # Load 100, then we'll pick successful ones
    ...
    try:
        stmt = (
            ...
            .on_conflict_do_update(
                ...
                set_={"config_value": study.best_params, "updated_at": func.now()},
            )
        )
        session.execute(stmt)
        session.commit()
        logger.success(
            "✅ Successfully persisted bullpen calibration to SQL table: simulation_configs"
        )
    except Exception as e:
        session.rollback()
