...
def fix_game_dates():
    ...
    query = select(GameResultORM).where(
        GameResultORM.game_datetime.isnot(None)
    )
    ...
    for corr in corrections:
        ...
        # Update game_results
        session.execute(
            update(GameResultORM).where(GameResultORM.game_id == gid).values(game_date=new_d)
        )
        
        # Update pitch_events
        session.execute(
            update(PitchEventORM).where(PitchEventORM.game_id == gid).values(game_date=new_d)
        )
        
        # Update statcast_raw
        session.execute(
            update(StatcastRawORM).where(StatcastRawORM.game_pk == int(gid)).values(game_date=new_d)
        )
        
    logger.info("Game-level dates corrected. Feature tables (Usage/Rolling) require localized re-backfill.")
