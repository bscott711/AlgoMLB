...
for g in games:
    print(f"  game_id={g.game_id} {g.game_date} {g.away_team}@{g.home_team} score={g.away_score}-{g.home_score}")
...
print(f"Expected features count: {len(expected_features) if expected_features else 'N/A'}")
if expected_features:
    ...
    for f in expected_features:
        ...
        # Better grouping
        for p in ['batter_', 'pitcher_', 'h_bat_', 'a_bat_', 'cnt_', 'elo_', 'home_', 'away_']:
            if f.startswith(p):
                prefix = p
                break
...
if expected_features:
    ...
    for f_name in ["n_games_used", "window_games", "days_since_last_game", "fatigue_index_7d", "fatigue_index_14d"]:
    ...
    expected_mlb = {
        ...
        "strikeout": 0.227, "out_in_play": 0.437,
    }
    ...
    on_base_rate = sum(probs[0][outcome_map.index(o)] for o in ["single", "double", "triple", "home_run", "walk", "hbp"] if o in outcome_map)
    out_rate = sum(probs[0][outcome_map.index(o)] for o in ["strikeout", "out_in_play"] if o in outcome_map)
...
print(f"Home: mean={np.mean(home_scores):.2f}, std={np.std(home_scores):.2f}, range=[{min(home_scores)},{max(home_scores)}]")
print(f"Away: mean={np.mean(away_scores):.2f}, std={np.std(away_scores):.2f}, range=[{min(away_scores)},{max(away_scores)}]")
...
for b in ctx.home_lineup + ctx.away_lineup:
    ...
    print(f"  [{side}] {name:25s}: xR={xr[0]:.3f}  xH={xh[0]:.3f}" if len(xr) > 0 and len(xh) > 0 else f"  [{side}] {name:25s}: NO DATA")
...
for pid, state in r0.player_states.items():
    if isinstance(state, BSS):
        ...
        print(f"  {name:25s}: PA={state.pa_count} H={state.hits}(1B={state.singles} 2B={state.doubles} 3B={state.triples} HR={state.hr}) R={state.runs} RBI={state.rbi} BB={state.walks} K={state.strikeouts}")
