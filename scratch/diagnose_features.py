...
stmt = (
    ...
    .order_by(desc(PlayerRollingFeaturesORM.game_date))
    .limit(1)
)
...
if row:
    ...
    for col in row.__table__.columns:
        ...
        if val is not None and col.name not in [
            "id",
            "player_id",
            "game_date",
            "role",
...
for count in count_states:
    ...
    print(
        f"{count:>5s}  {p_out:>8.4f}  {p_k:>8.4f}  {p_bb:>8.4f}  {p_1b:>8.4f}  {p_2b:>8.4f}  {p_hr:>8.4f}  {obp:>8.4f}"
    )
...
obp_nc = sum(
    probs_no_cnt[outcome_map.index(o)]
    for o in ["single", "double", "triple", "home_run", "walk", "hbp"]
)
print(
...
print("\n\n=== KEY FINDING SUMMARY ===")
print(
    f"1. Model has {len(expected_features)} features, {len([f for f in expected_features if f.startswith('cnt_')])} are count features"
)
...
print(
    f"6. The count is making a {'BIG' if abs(walk_3_2 - walk_0_0) > 0.1 else 'small'} difference"
)
