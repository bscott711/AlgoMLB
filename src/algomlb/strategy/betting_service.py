...
class BettingService:
    ...
    def place_daily_bets(self, target_date: datetime.date, min_edge: float = 0.05, stake: float = 5.0):
        """Scan for +EV opportunities and place pending bets."""
        games = self.session.query(GameResultORM).filter(GameResultORM.game_date == target_date).all()
        
        placed_count = 0
        for game in games:
            # Check if we already have a bet for this game
            existing = self.session.query(BankrollLedgerORM).filter(BankrollLedgerORM.game_id == str(game.game_id)).first()
            if existing:
                continue
            ...
            try:
                ...
                if market_odds:
                    ...
                    if abs(edge) >= min_edge:
                        ...
                        final_odds = market_odds.price if market_odds.outcome == selection else (1.0 / (1.0 - implied_prob))
    ...
    def settle_bets(self):
        ...
        for bet in pending_bets:
            game = self.session.query(GameResultORM).filter(GameResultORM.game_id == bet.game_id).first()
