...
def _transform_hook_row(row, game_map, retro_to_mlb) -> dict | None:
    ...
    try:
        ...
        return {
            ...
            "runs_allowed": int(row["runs_allowed"]) if "runs_allowed" in row.index else None,
            "hits_allowed": int(row["hits_allowed"]) if "hits_allowed" in row.index else None,
            "walks_allowed": int(row["walks_allowed"]) if "walks_allowed" in row.index else None,
            "strikeouts": int(row["strikeouts"]) if "strikeouts" in row.index else None,
