...
def _load_table_data(table_name: str, fk_filter: tuple[str, str | int | float | bool] | None = None, page: int = 0, page_size: int = 100) -> pd.DataFrame:
    engine = get_engine()
    where_clause = ""
    params = {}
    if fk_filter:
    ...
    query = text(f"SELECT * FROM \"{table_name}\" {where_clause} LIMIT {page_size} OFFSET {offset}")
    ...
    with col_main:
        ...
        with tab_data:
            ...
            try:
                ...
                if null_cols:
                    st.warning(f"⚠️ **Data Quality Alert:** The following columns are 100% NULL: `{', '.join(null_cols)}`")
                ...
                if not fks:
                    ...
                    for i, fk in enumerate(fks):
                        with fk_cols[i % 4]:
                            if st.button(f"Go to {fk.to_table} (via {fk.from_col})", key=f"fk_nav_{i}"):
