...
def _render_table_header(selected_table: str, tables: list):
...
def _render_pagination(table_meta, page_size: int):
    total_pages = 1
...
def _render_fk_navigation(selected_table: str, inspector: SchemaInspector):
    st.markdown("---")
    st.subheader("🔗 Relations (Foreign Keys)")
    fks = [fk for fk in inspector.foreign_keys() if fk.from_table == selected_table]
    if not fks:
        ...
        for i, fk in enumerate(fks):
            with fk_cols[i % 4]:
                if st.button(
                    f"Go to {fk.to_table} (via {fk.from_col})", key=f"fk_nav_{i}"
                ):
...
def _render_data_tab(selected_table: str, table_meta, inspector: SchemaInspector):
    page_size = 100
    _render_pagination(table_meta, page_size)
...
def _render_schema_tab(selected_table: str, inspector: SchemaInspector):
...
def render_db_explorer():
    ...
    with col_nav:
        _render_sidebar(inspector, tables)
    ...
    with col_main:
        ...
        table_meta = _render_table_header(selected_table, tables)
        tab_data, tab_schema = st.tabs(["📁 Data Viewer", "📋 Schema Dictionary"])

        with tab_data:
            _render_data_tab(selected_table, table_meta, inspector)

        with tab_schema:
            _render_schema_tab(selected_table, inspector)
