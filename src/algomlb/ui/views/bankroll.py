...
def render_bankroll_view():
    ...
    try:
        ...
        with tab1:
            # 1. Calculate Metrics
            total_balance = (
                session.query(func.sum(BankrollLedgerORM.pnl)).scalar() or 0.0
            )
            pending_count = (
            ...
            settled_count = (
                session.query(BankrollLedgerORM)
                .filter(BankrollLedgerORM.status == TransactionStatus.SETTLED)
                .count()
            )
            ...
            wins = (
                session.query(BankrollLedgerORM)
                .filter(
                    BankrollLedgerORM.status == TransactionStatus.SETTLED,
                    BankrollLedgerORM.pnl > 0,
                )
                .count()
            )
            ...
            ledger_query = (
                ...
                .outerjoin(
                    GameResultORM, BankrollLedgerORM.game_id == GameResultORM.game_id
                )
                    ...
                    data.append(
                        {
                            ...
                            "Stake": f"${t.stake:.2f}",
                            "Odds": f"{t.odds:.2f}",
                            "Edge": f"{getattr(t, 'edge', 0.0):+.1%}"
                            if getattr(t, "edge", None) is not None
                            else "N/A",
        ...
        with tab2:
            ...
            st.markdown(
                "Tracks the performance of random parlays and Play of the Day (POTD) slips separately with isolated bankroll management."
            )

            # Fetch degen/potd slips
            slips_query = text(
                "SELECT slip_id, slip_type, legs, final_odds, stake, status, pnl, created_at, settled_at FROM fadegoblin_slips ORDER BY created_at DESC;"
            )
            ...
            if slips:
                ...
                for s in slips:
                    ...
                    legs_summary = " | ".join(
                        [
                            f"{leg.get('pick', 'N/A')} ({leg.get('odds', 'N/A')})"
                ...
                if settled_count > 0:
                    ...
                    settled_chronological = sorted(
                        settled_slips, key=lambda x: x[7] if x[7] else x[8]
                    )
                    dates = [s[7] for s in settled_chronological]
                    pnls = [
                        float(s[6])
                        for s in settled_chronological
                        if float(s[6]) is not None
                    ]
                ...
                st.info(
                    "No FadeGoblin slips found in the ledger. Once the bot posts a degen parlay or POTD preview, they will show up here!"
                )
