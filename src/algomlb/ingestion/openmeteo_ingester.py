...
MLB_STADIUM_COORDS: dict[str, tuple[float, float]] = {
    ...
    "TB":  (27.7682, -82.6534),  "TEX": (32.7473, -97.0818),
...
class OpenMeteoIngester:
    ...
    def ingest_range(self, start_date: datetime.date, end_date: datetime.date) -> None:
        ...
        with self.session_factory() as session:
            ...
            for g, bp in games:
                y = g.game_date.year
                if y not in games_by_year: games_by_year[y] = []
        ...
        for year in sorted(games_by_year.keys(), reverse=True):
            ...
            df_weather['ts_iso'] = df_weather['datetime_utc'].dt.strftime('%Y-%m-%dT%H:00:00Z')
            weather_lookup = df_weather.set_index(['stadium_code', 'ts_iso']).to_dict('index')
            ...
            with self.session_factory() as bulk_session:
                ...
                for game, ballpark in games_by_year[year]:
                    ...
                    for h in range(5):
                        ...
                        row = weather_lookup.get((code, ts_iso))
                        if row: rows.append(row)
                    
                    if len(rows) < 4: continue # Sequence safety
                    
                    # 🌬️ Enhanced Wind Physics
                    bearing = ballpark.hp_bearing_deg or 0.0
                    wc_t0 = wind_components(rows[0]["wind_speed"], rows[0]["wind_dir"], bearing)
                    wc_t3 = wind_components(rows[min(3, len(rows)-1)]["wind_speed"], rows[min(3, len(rows)-1)]["wind_dir"], bearing)
                    
                    bulk_session.merge(OpenMeteoWeatherProgressionORM(
                        game_id=game.game_id,
                        **{f"temp_t{i}_f": rows[i]["temp"] for i in range(len(rows))},
                        **{f"wind_speed_t{i}": rows[i]["wind_speed"] for i in range(len(rows))},
                        **{f"wind_dir_t{i}": rows[i]["wind_dir"] for i in range(len(rows))},
                        humidity_t0=rows[0]["humidity"], precip_t0_mm=rows[0]["precip"],
                        pressure_t0_hpa=rows[0]["pressure"], cloud_cover_t0_pct=rows[0]["cloud_cover"],
                        temp_delta_game=rows[min(3, len(rows)-1)]["temp"] - rows[0]["temp"],
                        temp_min_game=min(r["temp"] for r in rows),
                        wind_speed_max_game=max(r["wind_speed"] for r in rows),
                        wind_dir_variance_deg=circular_std([r["wind_dir"] for r in rows]),
                        ...
                        wind_shift_gt_45deg=circular_std([r["wind_dir"] for r in rows]) > 45.0,
                        temp_drop_gt_10f=(rows[min(3, len(rows)-1)]["temp"] - rows[0]["temp"]) < -10.0,
                        precip_any_game=any(r["precip"] > 0.1 for r in rows),
                        forecast_temp_f=rows[0]["temp"], forecast_wind_speed_mph=rows[0]["wind_speed"],
                        forecast_wind_dir_deg=rows[0]["wind_dir"], forecast_headwind_mph=wc_t0.headwind_mph,
                        forecast_crosswind_mph=wc_t0.crosswind_mph, forecast_precip_prob=float(any(r["precip"] > 0.1 for r in rows)),
                    ...
                    inserted += 1
                    if inserted % 200 == 0: bulk_session.commit()
