...
def profiles():
    return [
        RelieverProfile(
            player_id=1, hand="R", role="closer", rest_days=2, pitches_yesterday=0
        ),
        RelieverProfile(
            player_id=2, hand="L", role="setup", rest_days=1, pitches_yesterday=15
        ),
        RelieverProfile(
            player_id=3, hand="R", role="middle", rest_days=0, pitches_yesterday=45
        ),
        RelieverProfile(
            player_id=4, hand="L", role="loogy", rest_days=1, pitches_yesterday=10
        ),
        RelieverProfile(
            player_id=5, hand="R", role="long", rest_days=3, pitches_yesterday=0
        ),
    ]
...
def test_select_next_high_leverage(profiles, rng):
    ...
    selected = orch.select_next(
        available, upcoming_batter_hands=["R"], leverage_index=2.0, game_inning=9
    )
...
def test_select_next_platoon_match(profiles, rng):
    ...
    selected = orch.select_next(
        available, upcoming_batter_hands=["L"], leverage_index=1.0, game_inning=7
    )
